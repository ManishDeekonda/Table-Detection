...
from train_file import config_util
...
class ConfigUtilTest(tf.test.TestCase):
  ...
  def testNewClassificationLocalizationWeightRatio(self):
    ...
    self.assertAlmostEqual(new_weight_ratio, loss.second_stage_classification_loss_weight)
  ...
  #def testNewFocalLossParameters(self):
    ...
    pipeline_config = pipeline_pb2.TrainEvalPipelineConfig()
    classification_loss = pipeline_config.model.faster_rcnn.second_stage_classification_loss_weight
    ...
    configs = config_util.get_configs_from_pipeline_file(pipeline_config_path)
    configs = config_util.merge_external_params_with_configs(configs, hparams)
    classification_loss = configs["model"].faster_rcnn.second_stage_classification_loss_weight
