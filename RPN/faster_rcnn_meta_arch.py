...
from train_file import balanced_positive_negative_sampler as sampler
from anchor_generator import boxlist
from intermediate import box_list_ops
from train_file import box_predictor
from intermediate import losses
from intermediate import model
from train_file import post_processing
from intermediate import standard_fields as fields
from train_file import target_assigner
...
class FasterRCNNMetaArch(model.DetectionModel):
  ...
  def __init__(self,
    ...
    Args:
      ...
      first_stage_anchor_generator: An anchor_generator.AnchorGenerator object
        ...
        anchor_generator.GridAnchorGenerator objects)
  ...
  def anchors(self):
    ...
    if not isinstance(self._anchors, boxlist.BoxList):
  ...
  def _postprocess_rpn(self,
    ...
    def normalize_boxes(args):
      ...
      normalized_boxes_per_image = box_list_ops.to_normalized_coordinates(
          boxlist.BoxList(proposal_boxes_per_image), image_shape[0],
  ...
  def _unpad_proposals_and_sample_box_classifier_batch(
    ...
    for (single_image_proposal_boxes,
      ...
      single_image_proposal_scores = tf.slice(single_image_proposal_scores,
                                              [0],
                                              [single_image_num_proposals])
      single_image_boxlist = boxlist.BoxList(single_image_proposal_boxes)
  ...
  def _format_groundtruth_data(self, true_image_shapes):
    ...
    groundtruth_boxlists = [
        box_list_ops.to_absolute_coordinates(
            boxlist.BoxList(boxes), true_image_shapes[i, 0],
  ...
  def _batch_decode_boxes(self, box_encodings, anchor_boxes):
    ...
    tiled_anchors_boxlist = boxlist.BoxList(
    ...
    with tf.name_scope('RPNLoss'):
      (batch_cls_targets, batch_cls_weights, batch_reg_targets,
       batch_reg_weights, _) = target_assigner.batch_assign_targets(
           self._proposal_target_assigner, boxlist.BoxList(anchors),
           groundtruth_boxlists,
  ...
  def _loss_box_classifier(self,
    ...
    with tf.name_scope('BoxClassifierLoss'):
      ...
      proposal_boxlists = [
          boxlist.BoxList(proposal_boxes_single_image)
      ...
      if prediction_masks is not None:
        ...
        flat_normalized_proposals = box_list_ops.to_normalized_coordinates(
            boxlist.BoxList(tf.reshape(proposal_boxes, [-1, 4])),
  ...
  def _unpad_proposals_and_apply_hard_mining(self,
    ...
    for (proposal_boxlist, single_image_loc_loss, single_image_cls_loss,
         single_image_num_proposals) in zip(
             ...
             tf.unstack(num_proposals)):
      proposal_boxlist = boxlist.BoxList(
